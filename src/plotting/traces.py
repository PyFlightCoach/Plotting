...
def meshes(
    npoints, seq: State | Transformation, colour: str = None, scale=1, _obj: OBJ = None
):
    ...
    if npoints >= 3:
        locs = locs + list(
            np.cumsum(np.full(npoints - 2, len(seq) / (npoints - 1))).astype(int)
        )
    ...
    for i, loc in enumerate(locs):
        ms.append(
            _obj.transform(
                ...
                colour or "grey",
                f"{(seq.time.t[loc] if isinstance(seq, State) else i):.1f}",
            )
        )
    return ms
...
def cgtrace(seq, **kwargs):
    return trace3d(*seq.pos.data.T, **kwargs)
...
def tiptrace(seq, span, line=None, **kwargs):
    ...
    def make_offset_trace(pos, colour):
        tr = trace3d(
            ...
            **dict(
                line=dict(color=colour, width=1) | ({} if line is None else line),
                **kwargs,
            ),
        )
        tr["showlegend"] = False
...
def sec_col_trace(
    sec, columns, dash="solid", colours=px.colors.qualitative.Plotly, yfunc=lambda x: x
):
    trs = []
...
def control_input_trace(
    sec, dash="solid", colours=px.colors.qualitative.Plotly, control_inputs=None
):
...
def arrow3d(
    ...
    if width>0:
        traces.append(go.Scatter3d(
            x=[origin.x[0], tip.x[0]],
            y=[origin.y[0], tip.y[0]],
            z=[origin.z[0], tip.z[0]],
            mode="lines",
